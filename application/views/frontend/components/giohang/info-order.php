...
<section id="checkout-cart">
    <div class="container">
        <div class="col-md-12">
                                ...
                                <table class="table tinfo" style="width: 80%;">
                                    <tbody>
                                        <tr>
                                            ...
                                            <td>
                                                <input type="text" class="form-control" placeholder="Họ và tên" name="name" value="<?php echo $user["fullname"]; ?>" <?php if (
                                                    $this->session->userdata("sessionKhachHang")
                                                ) {
                                                    echo "readonly";
                                                } ?>>
                                                <div class="error"><?php echo form_error(
                                                                        "name"
                                                                    ); ?></div>
                                            </td>
                                        </tr>
                                        <tr>
                                            ...
                                            <td>
                                                <input type="text" class="form-control" name="<?php if (
                                                                                                    ...
                                                                                                    echo "email";
                                                                                                } ?>" value="<?php echo $user["email"]; ?>" placeholder="Email" <?php if (
                                $this->session->userdata("sessionKhachHang")
                            ) {
                                echo "readonly";
                            } ?>>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <input type="text" class="form-control" placeholder="Số điện thoại" name="phone" value="<?php echo $user["phone"]; ?>" <?php if (
                                                    $this->session->userdata("sessionKhachHang")
                                                ) {
                                                    echo "readonly";
                                                } ?>>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <select name="city" id="province" onchange="renderDistrict()" class="form-control next-select">
                                                    ...
                                                    <?php
                                                    $list = $this->Mprovince->province_all();
                                                    foreach ($list as $row) : ?>
                                ...
                                <tbody>
                                    ...
                                    <?php if ($this->session->userdata("cart")) :
                                        ...
                                        foreach ($data as $key => $value) :
                                            $row = $this->Mproduct->product_detail_id(
                                                $key
                                            ); ?>
                                            <tr>
                                    ...
                                    <td>
                                        <tr>
                                            ...
                                            <td style="float: right;"><?php echo number_format(
                                                                            $money
                                                                        ); ?> VNĐ</td>
                                        </tr>
                                    </td>
                                    <tr>
                                        ...
                                        <td style="float: right;"><?php echo number_format(
                                                                        $this->Mconfig->config_price_ship()
                                                                    ) . " VNĐ"; ?> </td>
                                    </tr>

                                    <?php if ($this->session->userdata("coupon_price")) {
                                        $price_coupon_money = $this->session->userdata(
                                            "coupon_price"
                                        );
                                        $price_coupon = number_format(
                                            $this->session->userdata("coupon_price")
                                        );
                                        $price_coupon_percent = $this->session->userdata(
                                            "coupon_percent"
                                        );
                                        $is_percent_coupon = $this->session->userdata(
                                            "is_percent_coupon"
                                        );
                                        $coupon = "";
                                        if (
                                            isset($is_percent_coupon) &&
                                            $is_percent_coupon == 1
                                        ) {
                                            $coupon = $price_coupon_percent . " % ";
                                        } else {
                                            $coupon = $price_coupon . " VNĐ";
                                        }
                                        echo '
            ...
            <!-- <form onsubmit="return confirm('Xác nhận thanh toán ONLINE ')" method="POST" action="<?php echo base_url(
                                                                                                        "online-checkout"
                                                                                                    ); ?>"> -->
                <div class="btn-tks clearfix">
                    <td style="border: none;">
                        <div class="btn-checkout frame-100-1 overflow-hidden border-pri" style="float: right;">
                            <!--<button type="submit" name="payUrl" class="btn-next-checkout pull-left">Thanh toán MOMO</button> -->
                            <button type="submit" onclick="return confirm('Xác nhận thanh toán ONLINE ')" name="redirect" class="btn-next-checkout pull-left" id="payment_vnp">Thanh toán VNPAY</button>
                            <input type="text" id="payment_method" name="payment_method" hidden>
                    </td>
            <!-- </form> -->
        </div>
    </div>
